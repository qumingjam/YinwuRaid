...
public class RewardEntry {
            ...
            if (parts.length >= 1) {
                try {
                    Enchantment enchantment = Enchantment.getByKey(
                        new org.bukkit.NamespacedKey("minecraft", parts[0].toLowerCase())
                    );
            ...
            if (obj instanceof Map) {
                ...
                for (Map.Entry<String, Object> entry : enchantMap.entrySet()) {
                    try {
                        ...
                        Enchantment enchantment = Enchantment.getByKey(
                            new org.bukkit.NamespacedKey("minecraft", enchantName.toLowerCase())
                        );
    ...
    private static List<org.bukkit.enchantments.Enchantment> loadExcludedEnchantments(ConfigurationSection section) {
        ...
        for (String enchantName : excludedList) {
            ...
            try {
                Enchantment enchantment = Enchantment.getByKey(
                    new org.bukkit.NamespacedKey("minecraft", enchantName.toLowerCase().trim())
                );
