...
public class DisasterSeedManager {
    ...
    private final NamespacedKey enhanceCountKey;
    private final NamespacedKey seedUsageKey;
    private final Map<String, Integer> enchantmentLimits = new ConcurrentHashMap<>();
    ...
    public ItemStack performEnhancement(ItemStack targetItem, ItemStack seedItem) {
        ...
        // 获取可强化的自定义附魔（YinwuEnchant 联动）
        List<net.yinwu.lib.api.EnchantAPI.EnchantmentInstance> baseCustomEnchants = new ArrayList<>();
        if (enchantAPI != null) {
            for (var ce : enchantAPI.getEnchantments(targetItem)) {
                if (ce.level() < enchantAPI.getMaxLevel(ce.id())) {
                    baseCustomEnchants.add(ce);
                }
            }
        }

        if (baseEnchants.isEmpty() && baseCustomEnchants.isEmpty()) return null;
        ...
        for (int i = 0; i < times; i++) {
            ...
            // 自定义附魔可用池（YinwuEnchant 联动）
            List<String> availableCustom = new ArrayList<>();
            if (enchantAPI != null) {
                for (var ce : enchantAPI.getEnchantments(targetItem)) {
                    if (ce.level() < enchantAPI.getMaxLevel(ce.id())) {
                        availableCustom.add(ce.id());
                    }
                }
            }
            ...
            // 有自定义附魔时，30% 概率增强自定义附魔
            if (!availableCustom.isEmpty() && (availableEnchants.isEmpty() ||
                    ThreadLocalRandom.current().nextDouble() < 0.30)) {
                String selected = availableCustom.get(
                    ThreadLocalRandom.current().nextInt(availableCustom.size()));
                int currentLevel = enchantAPI.getEnchantmentLevel(targetItem, selected);
                enchantAPI.applyEnchantment(targetItem, selected, currentLevel + 1);
                enhanced = true;

                if (plugin.getConfigManager().isDebugEnabled()) {
                    plugin.getLogger().info("§e[DEBUG] [联动] 灾厄强化自定义附魔: " + selected + " → " + (currentLevel + 1));
                }
            }
            ...
            if (!enhanced) {
                ...
                Enchantment selected = availableEnchants.get(
                    ThreadLocalRandom.current().nextInt(availableEnchants.size()));
    ...
    private int calculateEnhancementTimes(ItemStack seed) {
        ...
        org.yinwu.config.EnhancementConfig enhancementConfig =
