...
public class EnchantmentRuleManager {
    ...
    // ✅ 从配置加载的附魔规则 - 改为实例变量
    private Set<Enchantment> toolCommon = ConcurrentHashMap.newKeySet();
    private Set<Enchantment> pickaxeExclusive = ConcurrentHashMap.newKeySet();
    private Set<Enchantment> shovelExclusive = ConcurrentHashMap.newKeySet();
    private Set<Enchantment> axeAllowed = ConcurrentHashMap.newKeySet();
    private Set<Enchantment> hoeAllowed = ConcurrentHashMap.newKeySet();
    private Set<Enchantment> weaponCommon = ConcurrentHashMap.newKeySet();
    private Set<Enchantment> bowAllowed = ConcurrentHashMap.newKeySet();
    private Set<Enchantment> crossbowAllowed = ConcurrentHashMap.newKeySet();
    private Set<Enchantment> tridentAllowed = ConcurrentHashMap.newKeySet();
    private Set<Enchantment> fishingRodAllowed = ConcurrentHashMap.newKeySet();
    private Set<Enchantment> shieldAllowed = ConcurrentHashMap.newKeySet();
    private Set<Enchantment> elytraAllowed = ConcurrentHashMap.newKeySet();
    private Set<Enchantment> armorGeneric = ConcurrentHashMap.newKeySet();
    private Set<Enchantment> helmetExclusive = ConcurrentHashMap.newKeySet();
    private Set<Enchantment> bootsExclusive = ConcurrentHashMap.newKeySet();

    // ✅ 互斥附魔规则（CopyOnWriteArrayList 保证迭代时并发修改安全）
    private List<Set<Enchantment>> mutuallyExclusiveGroups = new java.util.concurrent.CopyOnWriteArrayList<>();
    ...
    private void loadRulesFromConfig() {
        ...
        org.yinwu.config.EnhancementConfig enhancementConfig = configManager.getEnhancementConfig();
    ...
    private Set<Enchantment> loadEnchantmentSet(org.bukkit.configuration.ConfigurationSection section, String path) {
        ...
        Set<Enchantment> enchantSet = ConcurrentHashMap.newKeySet();
        ...
        for (String name : enchantNames) {
            try {
                ...
                Enchantment enchant = Enchantment.getByKey(new org.bukkit.NamespacedKey("minecraft", keyName));
    ...
    private void loadMutuallyExclusiveRules(org.bukkit.configuration.ConfigurationSection rulesSection) {
        ...
        for (String groupKey : exclusiveSection.getKeys(false)) {
            ...
            Set<Enchantment> group = ConcurrentHashMap.newKeySet();
    ...
    private void useDefaultRules() {
        ...
        // 默认互斥规则
        mutuallyExclusiveGroups.clear();
        mutuallyExclusiveGroups.add(Set.of(Enchantment.FORTUNE, Enchantment.SILK_TOUCH));
        mutuallyExclusiveGroups.add(Set.of(Enchantment.FROST_WALKER, Enchantment.DEPTH_STRIDER));
